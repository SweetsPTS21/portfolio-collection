...
test('portfolio renders the botanical garden experience', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Giang Lê Phương Linh' })).toBeVisible();
  await expect(
    page
  ...
  await expect(page.getByRole('heading', { level: 2, name: 'DỰ ÁN' })).toBeVisible();
  await expect(page.getByRole('heading', { level: 2, name: 'TỔNG KẾT' })).toBeVisible();
  await expect(page.getByText('WELCOME TO MY PORTFOLIO')).toBeVisible();
  await expect(page.getByText('sinh viên năm hai chuyên ngành Luật Học')).toBeVisible();
  await expect(page.getByRole('heading', { name: 'Bài 1: Máy tính và các thiết bị ngoại vi' })).toBeVisible();
  await expect(page.getByRole('heading', { name: 'Bài 6: An toàn và liêm chính học thuật' })).toBeVisible();
  await expect(page.getByText('Cơ sở dữ liệu quốc gia về văn bản pháp luật')).toBeVisible();
  await expect(page.getByText('AI ảo tưởng có thể tạo ra các điều luật hoặc án lệ giả mạo')).toBeVisible();
  await expect(page.getByText('Phương Linh chân thành cảm ơn Thầy/Cô')).toBeVisible();
  await expect(page.getByText('© 2026 • Giang Lê Phương Linh • Digital Portfolio')).toBeVisible();

  const viewButtons = page.getByRole('button', { name: /Xem bài/i });
  await expect(viewButtons).toHaveCount(6);
  await viewButtons.first().click();

  const pdfDialog = page.getByRole('dialog', { name: 'Bài 1: Máy tính và các thiết bị ngoại vi' });
